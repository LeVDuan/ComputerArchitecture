...
main:	la	$t1, s1		# lưu địa chỉ chuỗi s1 để lưu nội dung chuỗi sẽ được nhập vào
	...
	jal	count_common	# count_common($a0, $a1, $k1) đếm số lần lặp lại của các kí tự chung 
	nop
...
loop_stack:	beq $t1, $k1, print_result 	# nếu i == số lượng phần tử stack -> dừng và in kết quả
		...
		lb $s3, 0($t2)		# load phần tử thứ i vào $s3
...
print_result: 	li, $v0, 4
		...
		add $ra, $t9, $0 	# Khôi phục địa chỉ trả về $ra = $t9
		jr $ra
...
count_in_string:	li $t3, 0 	# biến chạy i = 0
			li $k0, 0 	# gán lại giá trị trả về $k0 = 0
...
end_count_in_string: 	jr $ra	# Thoát hàm
